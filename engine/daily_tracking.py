...
def load_histo_export(source: str | BinaryIO | BytesIO) -> pd.DataFrame:
    ...
    if tel_col not in df.columns and "TEL" in df.columns:
        tel_col = "TEL"
...
def filter_baseline_cohort(
    ...
    *,
    ...
    labels = status_labels if status_labels is not None else DEFAULT_COHORT_LABELS
    out = baseline.copy()
...
def compute_cohort_evolution(
    ...
    *,
    ...
    if data_client is not None and not data_client.empty:
        ...
        tel_col = _find_column(dc, TEL_ALIASES) or "TEL"
    ...
    still_same = int((detail["devenu_histo"].astype(str) == baseline_label).sum())
